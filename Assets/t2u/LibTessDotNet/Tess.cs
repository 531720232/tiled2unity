...
namespace LibTessDotNet
{
	public class Tess
	{
		...
		private void GetIntersectData(MeshUtils.Vertex isect, MeshUtils.Vertex orgUp, MeshUtils.Vertex dstUp, MeshUtils.Vertex orgLo, MeshUtils.Vertex dstLo)
		{
			...
			VertexWeights(isect, orgUp, dstUp, out float w, out float w2);
			VertexWeights(isect, orgLo, dstLo, out float w3, out float w4);
			if (_combineCallback != null)
			{
				isect._data = _combineCallback(isect._coords, new object[4]
				{
					orgUp._data,
					dstUp._data,
					orgLo._data,
		...
		{
			MeshUtils.Edge next;
			for (MeshUtils.Edge edge = _mesh._eHead._next; edge != _mesh._eHead; edge = next)
			{
				next = edge._next;
				if (edge._Rface._inside != edge._Lface._inside)
				{
					MeshUtils.Edge edge2 = edge;
					edge2._winding = (edge2._Lface._inside ? value : (-value));
				}
